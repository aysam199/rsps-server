...
public class PacketSender {
	...
	/**
	 * Tells the client that a custom ability item has gone on cooldown, so it can
	 * render a countdown overlay on the inventory slot.
	 *
	 * @param itemId  the ability item id
	 * @param seconds the cooldown duration in seconds
	 */
	public PacketSender sendAbilityCooldown(int itemId, int seconds) {
		PacketBuilder out = new PacketBuilder(33);
		out.putShort(itemId);
		out.put(seconds);
		player.getSession().write(out);
		return this;
