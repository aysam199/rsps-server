...
public class UseItemPacketListener extends ItemIdentifiers implements PacketExecutor {
    ...
    private static void itemOnNpc(final Player player, Packet packet) {
        ...
        // Custom ability items are cast instantly (ranged), so handle before walking.
        // The Ability Master is the exception: using an ability item on him opens
        // its upgrade menu rather than casting at him.
        if (!(npc instanceof com.elvarg.game.entity.impl.npc.impl.AbilityMaster)
                && AbilityHandler.activate(player, npc, item)) {
            return;
        }
    ...
    private static void itemOnPlayer(Player player, Packet packet) {
        ...
        // Custom ability items are cast instantly (ranged), so handle before walking.
        if (AbilityHandler.activate(player, target, item)) {
            return;
        }
